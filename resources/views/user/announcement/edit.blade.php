...
@section('page', 'Edit Announcement')
@section('breadcrumbs')
    @php
        ...
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-body py-4">
                    <form action="{{ route('announcements.update', $news->id) }}" method="post" enctype="multipart/form-data">
                        @csrf
                        @method('patch')
                        <div class="row">
                            <div class="col-lg-8 mb-4">
                                <img src="{{ empty($news->thumbnail) ? asset('itrack/images/thumbnail.svg') : Storage::disk('public')->url('thumbnails/'.$news->thumbnail)  }}" 
                                    alt="" class="img-fluid border rounded" id="thumbnail-preview">
                            </div>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="datetime-local" class="form-control @error('start_at') border-danger @enderror" id="start_at" name="start_at" 
                                    value="{{ old('start_at', $news->start_at) }}" required>
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label" for="end_at">End At <span class="text-danger">*</span></label>
                                <input type="datetime-local" class="form-control @error('end_at') border-danger @enderror" id="end_at" name="end_at" 
                                    value="{{ old('end_at', $news->end_at) }}" required>
                            </div>
                            <div class="col-12 mb-3">
                                ...
                                <div class="form-check-size">
                                    <div class="form-check form-switch form-check-inline">
                                        <input type="checkbox" class="form-check-input check-size" id="status" name="status" value="1" role="switch"
                                            {{ old('status') == 1 || $news->status == 'draft' ? 'checked' : '' }}>
